...
GT_25m <- read.csv("04_GT_250mExtract_25m.csv")
...
GT_100m <- read.csv("04_GT_250mExtract_100m.csv")
...
lst <- list.files(path= "C:/working/GRD_Mask_Files/250m/01_GRD_Files",pattern='.grd$',full.names = T) # list the name of files in the specified path,
lst 
preds <- stack(lst)
preds
...
# Model Fitting
m250m_CORAL <- sdm( ~ depth + bscatter + slope + bpi25 + bpi3 + curv + hyp5,
             data = d_25m,
...
write.sdm(d_25m,'sdm_course_250m_CORAL.sdd', overwrite = TRUE)
write.sdm(m250m_CORAL,'sdm_course_250m_CORAL.sdm', overwrite = TRUE)

# MODEL EVALUATION
meta_250m <- getModelInfo(m250m_CORAL)
ev_250m_test <- getEvaluation(m250m_CORAL, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.dep')
ev_250m_eval <- getEvaluation(m250m_CORAL, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.indep')
...
model <- 1:300
for (i in model) {
  vi <- getVarImp(m250m_CORAL, id = i)
  if(is.null(vi) == FALSE) importance <- vi@varImportance$AUCtest else importance <- NA
  vi_df[i,] <- importance  
}
vi_df$modelID <- meta_250m$modelID

# combine and export evaluation data
merge <- merge(meta_250m, ev_250m_test, by.x = "modelID", by.y = "modelID", all.x = T)
merge2 <- merge(merge, ev_250m_eval, by.x = "modelID", by.y = "modelID", all.x = T)
merge3 <- merge(merge2, vi_df, by.x = "modelID", by.y = "modelID", all.x = T)
merge3$Scenario <- "250 m"
colnames(merge3) <- gsub(".x", ".test", colnames(merge3))
setwd("C:/working/sdm_outputs_update/evaluation")
write.csv(merge3, "Evaluation_250m_CORAL.csv", row.names = F)

# PREDICT MODEL
setwd("C:/working/sdm_outputs_update/predictions")
predict(m250m_CORAL, preds, filename = 'predictions_250m_CORAL.img', mean = TRUE)

# ENSEMBLES
setwd("C:/working/sdm_outputs_update/predictions")
inf <- getModelInfo(m250m_CORAL)
ev <- getEvaluation(m250m_CORAL, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe'), opt = 2)
inf_ev <- merge(inf, ev, all.x = T)

CORAL <- inf$modelID[inf$species == "CORAL"]

# build using all models
e_all_CORAL <- ensemble(m250m_CORAL, preds, filename = 'ensemble_250m_coral_all.img',
                        setting = list(id = CORAL, method = 'weighted', stat = 'AUC'), overwrite = TRUE)

# keep those with AUC > 0.7, n = 49
id <- inf_ev$modelID[which(inf_ev$AUC >= 0.7 & inf_ev$species == "CORAL")]
e_AUC07_CORAL <- ensemble(m250m_CORAL, preds, filename = 'ensemble_250m_coral_AUC07.img',
...
# Model Fitting
m250m_MACRO <- sdm( ~ depth + bscatter + slope + bpi25 + bpi3 + curv + hyp5,
                    data = d_100m,
                    methods = c('brt', 'rpart', 'glm', 'mars', 'maxent', 'rf'),
                    replication = c('cv'),
                    cv.folds = 5,
...
# to save data and model products
setwd("C:/working/sdm_outputs_update/sdm_files")
write.sdm(d_100m,'sdm_course_250m_MACRO.sdd', overwrite = TRUE)
write.sdm(m250m_MACRO,'sdm_course_250m_MACRO.sdm', overwrite = TRUE)

# MODEL EVALUATION
meta_250m <- getModelInfo(m250m_MACRO)
ev_250m_test <- getEvaluation(m250m_MACRO, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.dep')
ev_250m_eval <- getEvaluation(m250m_MACRO, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.indep')
...
colnames(merge3) <- gsub(".x", ".test", colnames(merge3))
setwd("C:/working/sdm_outputs_update/evaluation")
write.csv(merge3, "Evaluation_250m_MACRO.csv", row.names = F)

# PREDICT MODEL
setwd("C:/working/sdm_outputs_update/predictions")
predict(m250m_MACRO, preds, filename = 'predictions_250m_MACRO.img', mean = TRUE)

# ENSEMBLES
setwd("C:/working/sdm_outputs_update/predictions")
inf <- getModelInfo(m250m_MACRO)
ev <- getEvaluation(m250m_MACRO, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe'), opt = 2)
inf_ev <- merge(inf, ev, all.x = T)

MACRO <- inf$modelID[inf$species == "MACRO"]

# build using all models
e_all_MACRO <- ensemble(m250m_MACRO, preds, filename = 'ensemble_250m_MACRO_all.img',
                        setting = list(id = MACRO, method = 'weighted', stat = 'AUC'), overwrite = TRUE)

# keep those with AUC > 0.7, n = 134
id2 <- inf_ev$modelID[which(inf_ev$AUC >= 0.7 & inf_ev$species == "MACRO")]
e_AUC07_MACRO <- ensemble(m250m_MACRO, preds, filename = 'ensemble_250m_MACRO_AUC07.img',
...
# Model Fitting
m250m_SPONGE <- sdm( ~ depth + bscatter + slope + bpi25 + bpi3 + curv + hyp5,
                    data = d_100mSP,
                    methods = c('brt', 'rpart', 'glm', 'mars', 'maxent', 'rf'),
                    replication = c('cv'),
                    cv.folds = 5,
...
# to save data and model products
setwd("C:/working/sdm_outputs_update/sdm_files")
write.sdm(d_100mSP,'sdm_course_250m_SPONGE.sdd', overwrite = TRUE)
write.sdm(m250m_SPONGE,'sdm_course_250m_SPONGE.sdm', overwrite = TRUE)

# MODEL EVALUATION
meta_250m <- getModelInfo(m250m_SPONGE)
ev_250m_test <- getEvaluation(m250m_SPONGE, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.dep')
ev_250m_eval <- getEvaluation(m250m_SPONGE, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe','threshold'), opt = 2, wtest = 'test.indep')
...
model <- 1:300
for (i in model) {
  vi <- getVarImp(m250m_SPONGE, id = i)
  if(is.null(vi) == FALSE) importance <- vi@varImportance$AUCtest else importance <- NA
  vi_df[i,] <- importance  
}
vi_df$modelID <- meta_250m$modelID
...
colnames(merge3) <- gsub(".x", ".test", colnames(merge3))
setwd("C:/working/sdm_outputs_update/evaluation")
write.csv(merge3, "Evaluation_250m_SPONGE.csv", row.names = F)

# PREDICT MODEL
setwd("C:/working/sdm_outputs_update/predictions")
predict(m250m_SPONGE, preds, filename = 'predictions_250m_SPONGE.img', mean = TRUE)

# ENSEMBLES
setwd("C:/working/sdm_outputs_update/predictions")
inf <- getModelInfo(m250m_SPONGE)
ev <- getEvaluation(m250m_SPONGE, stat = c('AUC', 'COR', 'TSS', 'Sens', 'Spe'), opt = 2)
inf_ev <- merge(inf, ev, all.x = T)

SPONGE <- inf$modelID[inf$species == "SPONGE"]

# build using all models
e_all_SPONGE <- ensemble(m250m_SPONGE, preds, filename = 'ensemble_250m_SPONGE_all.img',
                        setting = list(id = SPONGE, method = 'weighted', stat = 'AUC'), overwrite = TRUE)

# keep those with AUC > 0.7, n = 213
id3 <- inf_ev$modelID[which(inf_ev$AUC >= 0.7 & inf_ev$species == "SPONGE")]
e_AUC07_SPONGE <- ensemble(m250m_SPONGE, preds, filename = 'ensemble_250m_SPONGE_AUC07.img',
                          setting = list(id = id3, method = 'weighted', stat = 'AUC'), overwrite = TRUE)

setwd("C:/working/sdm_outputs_update/r_data")
# save.image(file = "sdm_250m_resolution.RData")
save.image(file = "sdm_250m_resolution.RData")
